...
void filterCloud(pcl::PointCloud<PointT> &cloud, map<int, int> &tablePoints,
                 string transformfile, pcl::PointIndices & objIndices,
  ...
  vector<int> indices;
  cout << "cloud size: " << cloud.points.size() << " indicies:"
...
void checkIndices(pcl::PointCloud<PointT> & cloud,
    ...
    for (int j = 0; j < 1; j++) {
     ...
     while (status > 0) {
        ...
        for (size_t o = 0; o < objData.size(); o++) {
          ...
          int rval = getObjectPointCloud(cloud, tablePoints, transformfile,
                                         objData.at(o), cloudInds, fn.str(),
                                         centroid);
          ...
          onum << o+1;
          if(rval == -1){
            cout << "ERROR IN BOUNDING BOX action: " << all_files[i]
                 << " obj :" << onum.str() << " fnum: " << status << endl;
          }

          string fname = dataLocation + "/objects/" + all_files.at(i)
                         + "_obj" +  onum.str() + ".txt";
